...
export async function seedPreview(repositories: Repositories): Promise<string> {
  ...
  const workspace = await repositories.workspaces.create({ name: "Tellus Cooperative" });
  ...
  await repositories.rules.create({
    ...
    name: "Leñería → Rent",
    priority: 10,
    conditions: [{ field: "contact", operator: "equals", value: contactByName("Leñería Alerce")! }],
    actions: [{ type: "set_category", value: byName("Rent") }],
  });
  await repositories.rules.create({
    workspaceId: workspace.id,
    name: "SDF incoming → Grants",
    priority: 20,
    conditions: [
      {
        field: "contact",
        operator: "equals",
        value: contactByName("Stellar Development Foundation")!,
      },
  ...
  await repositories.rules.create({
    workspaceId: workspace.id,
    name: "Alex → Contractors",
    priority: 30,
    conditions: [{ field: "contact", operator: "equals", value: contactByName("Alex Hernández")! }],
