...
describe("export", () => {
  it("carries the human layer and deliberately omits blockchain history", async () => {
    ...
    expect(backup.workspace.name).toBe("Tellus Cooperative");
  ...
  it("suggests a filename based on the ledger name", () => {
    expect(backupFilename("Tellus Cooperative", "stellarledger")).toBe(
      "Tellus Cooperative.stellarledger",
    );
...
describe("the full delete / reinstall / restore cycle", () => {
  it("restores metadata and reattaches it to a resynced ledger", async () => {
    ...
    expect(match!.contactName).toBe("Alex Hernández");
...
describe("CSV export", () => {
  ...
  it("quotes fields containing commas, quotes or newlines", () => {
    expect(escapeCsvField("Hernández, Alex")).toBe('"Hernández, Alex"');
