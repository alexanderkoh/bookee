...
beforeEach(async () => {
  ...
  const records = [
    payment("1", ALEX, SAMPLE_ACCOUNT, "1500.0000000", "2026-07-04T10:00:00Z"),
    payment("2", SAMPLE_ACCOUNT, ALEX, "1042.5000000", "2026-07-06T10:00:00Z"),
    payment("3", SAMPLE_ACCOUNT, RESERVE, "5000.0000000", "2026-07-20T10:00:00Z"),
    payment("4", ALEX, SAMPLE_ACCOUNT, "250.0000000", "2026-07-25T10:00:00Z", true),
  ];
...
describe("QuickBooks CSV", () => {
  ...
  it("describes a transaction with the human context the ledger holds", async () => {
    ...
    expect(csv).toContain("Alex Hernández");
  });
});
