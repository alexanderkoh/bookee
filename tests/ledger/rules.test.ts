...
function rule(overrides: Partial<ParsedRule> = {}): ParsedRule {
  return {
    ...
    enabled: true,
    priority: 100,
    conditions: [{ field: "counterparty_address", operator: "equals", value: EMILE }],
...
describe("condition matching", () => {
  ...
  it("requires every condition to match", () => {
    const both = rule({
      conditions: [
        { field: "counterparty_address", operator: "equals", value: EMILE },
...
describe("applying rules to a ledger", () => {
  ...
  it("never overwrites a category the user set by hand", async () => {
    ...
    const emileEntry = entries.find((entry) => entry.counterpartyAddress === EMILE)!;

    const categories = await repos.categories.list(workspace.id);
    const rentId = categories.find((category) => category.name === "Rent")!.id;

    await repos.annotations.setManual(emileEntry.id, { categoryId: rentId });
    ...
    const annotation = await repos.annotations.findByEntry(emileEntry.id);
